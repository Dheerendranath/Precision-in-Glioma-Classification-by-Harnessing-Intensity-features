...
main_directory = '/Users/dheerendranathbattalapalli/Desktop/BRATS_2018_dataset/MICCAI_BraTS_2018_Data_Training/HGG/';
subdirs = 1:150;
...
for idx = 1:length(subdirs)
    ...
    t1ce_file = dir(fullfile(folder_path, '*t1ce.nii.gz'));
    label_file = dir(fullfile(folder_path, '*seg.nii.gz'));
    
    % If the NIfTI files are missing, continue to the next folder
    if isempty(t1ce_file) || isempty(label_file)
        fprintf('Missing t1ce.nii.gz or seg.nii.gz in folder: %s\n', folder_path);
        continue;
    end
...
% Initialize the 3D skeleton
    skeleton = zeros(size(label_data));

% Perform morphological operations for each slice to extract the skeletal structure
    for slice = 1:size(label_data, 3)
        binary_slice = label_data(:,:,slice) > 0;
        skeleton(:,:,slice) = bwmorph(binary_slice, 'skel', Inf);
    end

% Overlay the skeletal structure on the T1ce image
    overlay_image = double(t1ce_data) .* double(skeleton);
    
    % Overlay the label data on the T1ce image
    if isa(t1ce_data, 'double')
        overlay_image = t1ce_data .* double(label_data);
    else
        overlay_image = int16(t1ce_data) .* int16(label_data);
    end

    
    % Extract the first-order statistical values from the ROI
    roi_pixels = overlay_image(label_data > 0);
